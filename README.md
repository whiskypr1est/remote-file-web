...
>
> ★ **所有「默认落在项目根目录」的状态文件都由脚手架重定向到临时目录**，
> 并且有一条结构性守卫盯着它：`DEFAULT_CONFIG` 里凡是默认值是 `.json` / `.jsonl`
> 文件的配置项，都必须登记在 `tests/_harness.py` 的 `STATE_PATH_KEYS` 中，
> 否则测试会直接失败。这个坑在本项目已经踩过三次（`config.json` 被覆盖导致
> 服务换端口失联、真实 `user_state.json` 里混进测试窗口、真实
> `desktop_shortcuts.json` / `audit.log.jsonl` 被写入），所以做成了机制。
