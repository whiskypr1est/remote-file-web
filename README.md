...
开始菜单 →「修改密码」，或桌面右键 →「修改密码…」。改完**所有已登录设备
（包括当前浏览器）都会退出**，需要用新密码重新登录。

这条顺带修掉了一条此前的已知限制：原先「修改密码不会让已登录的浏览器立即掉线」，
也就是口令一旦泄露，改密码并不能把已经进来的人赶出去。现在改成功会
**轮换会话密钥**，所有已签发的会话立即失效。另外，改密码**必须验证当前口令** ——
只看会话是不够的，Cookie 一旦被窃取，攻击者就能改掉口令把管理员永久锁在外面。
...
- **未实现**：批量重命名、**回收站查看与还原**（删除虽会进系统回收站，但 Web 界面里看不到已删除项，需到 Windows 回收站手动还原）、多用户与权限分级、操作审计日志、HTTPS（建议由反向代理承担）
